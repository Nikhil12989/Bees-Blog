            ...
            {userPosts.map((post) => (
              <Table.Body>
                <Table.Row className="bg-slate-50 dark:border-gray-700 dark:bg-gray-800">
                  ...
                  <Table.Cell> <Link to={`/post/${post.slug}`} className="font-medium ">{post.title}</Link></Table.Cell>
                  <Table.Cell>{post.category}</Table.Cell>
                  <Table.Cell> <span className="font-medium text-red-500" > <FaTrash /> </span> </Table.Cell>
                  <Table.Cell> <Link to={`/update-post/${post._id}`} className="text-teal-500"><span>  <FaEdit /></span></Link> </Table.Cell>
                </Table.Row>
              </Table.Body>
            ))}
          </Table>
